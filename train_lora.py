...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    text_trainable = lora_manager.use_text_lora
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if accelerator.sync_gradients:
                ...
                if should_sample(global_step, validation_steps, validation_data) and accelerator.is_main_process:
                    ...
                    text_encoder_module = accelerator.unwrap_model(text_encoder)
                    if global_step == 1: print("Performing validation prompt.")
                    with accelerator.autocast():
                        batch_eval(unet_module, text_encoder_module, vae, vae_processor, pretrained_model_path, 
                            validation_data, f"{output_dir}/samples", True, iters=1, lora_manager=lora_manager)
                    lora_manager.deactivate_lora_train([unet_module, text_encoder_module], False)    
...
def batch_eval(unet, text_encoder, vae, vae_processor, pretrained_model_path, 
    validation_data, output_dir, preview, global_step=0, iters=6, lora_rank=16, lora_path=None, lora_manager=None, unet_lora_modules=["UNet3DConditionModel"]):
    device = vae.device
    dtype = vae.dtype
    unet.eval()
    text_encoder.eval()
    ...
    # lora injection during eval
    if(lora_path and (lora_manager is None)):
        from utils.lora import inject_inferable_lora
        
        inject_inferable_lora(pipeline, lora_path, r=lora_rank, unet_replace_modules=unet_lora_modules) # , text_encoder_replace_modules=text_encoder_lora_modules
        # monkeypatch_remove_lora(unet)
        print(f'LoRA injected to {unet_lora_modules}, lora path: {lora_path}')
...
def main_eval(
    ...
    **kwargs
    ...
    batch_eval(unet, text_encoder, vae, vae_processor, pretrained_model_path, 
        validation_data, "output/demo", True, lora_rank=lora_rank, lora_path=lora_path, unet_lora_modules=unet_lora_modules)
